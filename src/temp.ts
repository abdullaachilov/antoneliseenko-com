...
function runBreathePhase(idx: number): void {
  ...
  const isCountPhase = idx === 0 || idx === 2;
...
function showQuizResult(): void {
  ...
  const level: 'low' | 'mid' | 'high' = score <= 1 ? 'low' : score <= 3 ? 'mid' : 'high';
...
const MOOD_RESPONSES: Record<string, string> = {
  great: 'temp_mood_response_great',
  okay: 'temp_mood_response_okay',
  anxious: 'temp_mood_response_anxious',
  sad: 'temp_mood_response_sad',
  empty: 'temp_mood_response_empty',
};

function initMoodCheckin(): void {
  ...
  if (!grid || !responseEl || !responseText) return;

  grid.querySelectorAll<HTMLButtonElement>('.mood-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      grid.querySelectorAll('.mood-btn').forEach((b) => b.classList.remove('selected'));
      btn.classList.add('selected');
      const mood = btn.dataset.mood!;
      const key = MOOD_RESPONSES[mood];
      responseText.textContent = t(key);
...
function refreshAffirmation(): void {
  const el = document.querySelector<HTMLElement>('.affirmation-text');
  if (!el) return;
  el.textContent = t(AFFIRMATION_KEYS[affirmationIdx]);
}

function initAffirmation(): void {
  // Pick daily affirmation based on day-of-year
  const now = new Date();
  const dayOfYear = Math.floor((now.getTime() - new Date(now.getFullYear(), 0, 0).getTime()) / 86400000);
  affirmationIdx = dayOfYear % AFFIRMATION_KEYS.length;
  refreshAffirmation();

  document.getElementById('affirmation-refresh')?.addEventListener('click', () => {
    affirmationIdx = (affirmationIdx + 1) % AFFIRMATION_KEYS.length;
    refreshAffirmation();
  });
}
...
function initTimelineDrag(): void {
  const wrap = document.querySelector<HTMLElement>('.timeline-scroll-wrap');
  if (!wrap) return;
  let isDown = false;
  let startX = 0;
  let scrollLeft = 0;
